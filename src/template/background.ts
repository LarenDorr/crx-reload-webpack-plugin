...
(function init(chrome, window: Window) {
	...
	function handleReload(types: string) {
		if (types===DIFF_CODE[1]) { // when manifest.json change, reloa extension
			chrome.runtime.reload()
		}
		if (types===DIFF_CODE[2]) { // when background js change, reload background tab
			let tab_id
		...
		if (types===DIFF_CODE[3]) {
			chrome.runtime.sendMessage({
				data: 'reloadPopup'
			})
		}
		if (types===DIFF_CODE[4]) { // when content js change, reload all tabs
